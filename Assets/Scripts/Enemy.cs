...
public class Enemy : MonoBehaviour
{
    ...
    // Legacy: kept for any prefab that manually assigns a shield child GO
    [SerializeField]
    private GameObject _enemyShield;
    private bool _isShielded = false;
    private SpriteRenderer _spriteRenderer;
    [SerializeField]
    private Color _shieldColor = new Color(0.3f, 0.7f, 1f, 1f); // blue tint when shielded
    ...
    void Start()
    {
        ...
        // If this prefab already has a legacy shield child assigned, activate the code-driven shield too
        if (_enemyShield != null)
            _isShielded = true;
        // Re-apply tint in case SetShieldActive was called before Start (e.g. from Spawn)
        if (_isShielded && _spriteRenderer != null)
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            // Laser handling is centralized through Laser.cs which calls TakeLaserHit(), keep this block minimal
            return;
        }
    ...
    {
        if(_playerLocation.position.y > transform.position.y && _playerLocation != null)
        {
    ...
    // Legacy: for prefabs that still wire up a child shield GO
    ...
    // Primary method: activates shield purely in code, no prefab needed
    public void SetShieldActive(bool active)
    {
        _isShielded = active;
        if (_spriteRenderer != null)
            _spriteRenderer.color = active ? _shieldColor : _originalColor;
    }

    private void StripShield()
    {
        _isShielded = false;
        if (_enemyShield != null) { Destroy(_enemyShield); _enemyShield = null; }
