...
public class Player : MonoBehaviour
{
    ...
    [SerializeField]
    private float _thrusterMax = 10f;
    ...
    void Update()
    {
        ...
        Shooting();
        if (_uiManager != null)
            _uiManager.DisplayThruster(_thrusterCurrent, _thrusterMax);
    ...
    void Thrusters()
    {
        ...
        if (Input.GetKey(KeyCode.LeftShift) && _thrusterCurrent > 0f)
        {
            _speed = _baseSpeed * _thrusterboost;
            _thrusterCurrent -= _thrusterDrainRate * Time.deltaTime;
            if (_thrusterCurrent <= 0f)
            {
                _thrusterCurrent = 0f;
                _speed = _baseSpeed;
            }
        ...
        {
            _speed = _baseSpeed;
            if (_thrusterCurrent < _thrusterMax)
            {
                _thrusterCurrent += _thrusterRechargeRate * Time.deltaTime;
                if (_thrusterCurrent > _thrusterMax) _thrusterCurrent = _thrusterMax;
            }
            if (_thrusterCurrent < _thrusterMax && _thrusterCurrent > 0)
    ...
    {
        // Deprecated: thrusters are now hold-to-use with fuel. Kept for compatibility but will immediately return.
        yield break;
    }
